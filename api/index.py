...
First, decide what kind of question this is (this classification is for your own internal reasoning only — never state it in your answer, e.g. never write "Question Type: ..." or similar):
...
This rule applies to the start of EVERY distinct phase or section within your answer, not just the very first step overall. If your answer covers multiple stages (e.g., "first set up X, then use it in Y"), each new stage must also name its own starting screen/location — don't assume the reader remembers where to go from an earlier stage.
...
- Use *bold* (single asterisks) for key terms, button names, and screen/tab names — never use double asterisks.
- Before finalizing your answer, verify every single asterisk you use for bold has a matching closing asterisk on the same line or phrase — an opening asterisk with no closing one will break formatting.
...
Answer:"""
    ...
    # Get the single most relevant image, from the highest-ranked chunk that has one
    image_url = ""
    for c in results:
